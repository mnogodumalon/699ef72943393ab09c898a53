...
interface UrlAnalyseDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: UrlAnalyse['fields']) => Promise<void>;
  defaultValues?: UrlAnalyse['fields'];
  enablePhotoScan?: boolean;
}

export function UrlAnalyseDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = false }: UrlAnalyseDialogProps) {
  const [fields, setFields] = useState<Partial<UrlAnalyse['fields']>>({});
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      await onSubmit(fields as UrlAnalyse['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handlePhotoScan(file: File) {
    ...
    try {
      ...
      const schema = `{\n  "eingabe_url": string | null, // URL eingeben\n  "original_link": string | null, // Extrahierter Originallink\n}`;
      ...
      setFields(prev => {
        ...
        return merged as Partial<UrlAnalyse['fields']>;
      });
      setScanSuccess(true);
  ...
  return (
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{defaultValues ? 'Edit URL-Analyse' : 'New URL-Analyse'}</DialogTitle>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-2">
            <Label htmlFor="eingabe_url">URL eingeben</Label>
            <Textarea
              id="eingabe_url"
              value={fields.eingabe_url ?? ''}
              onChange={e => setFields(f => ({ ...f, eingabe_url: e.target.value }))}
              rows={3}
            />
          </div>
          <div className="space-y-2">
            <Label htmlFor="original_link">Extrahierter Originallink</Label>
            <Textarea
              id="original_link"
              value={fields.original_link ?? ''}
              onChange={e => setFields(f => ({ ...f, original_link: e.target.value }))}
              rows={3}
            />
          </div>
          <DialogFooter>
